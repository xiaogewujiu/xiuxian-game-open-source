...
namespace XXX.Application.Services
{
    ...
    /// 炼丹职业规则运行时目录。
    /// </summary>
    internal static class AlchemyProfessionRuleRuntimeCatalog
    {
        private static readonly Dictionary<int, AlchemyProfessionLevelConfigEntity> LevelConfigs = new();
        private static AlchemyProfessionRuleConfigEntity? _ruleConfig;

        public static void ReplaceConfigs(
            IEnumerable<AlchemyProfessionLevelConfigEntity> levelConfigs,
            AlchemyProfessionRuleConfigEntity? ruleConfig)
        {
            LevelConfigs.Clear();
        ...
        public static int GetNextLevelExp(int currentLevel)
        {
            ...
            throw new InvalidOperationException(
                $"Alchemy profession level config {safeLevel} is missing. Please refresh the alchemy domain first.");
        }
        ...
        private static AlchemyProfessionRuleConfigEntity ResolveRuleConfig()
        {
            return _ruleConfig
                ?? throw new InvalidOperationException(
                    "Alchemy profession rule config has not been loaded. Please refresh the alchemy domain first.");
        }
    }
}
