  ...
  <div class="dashboard-grid">
    <section class="panel-box filter-panel">
      <div class="section-header-copy">
        <p class="section-kicker">炼丹规则</p>
        <h2 class="section-title section-title--small">炼丹规则</h2>
        <p class="section-note">维护炼丹师等级经验曲线和炼丹职业通用规则。</p>
      </div>
    ...
    <AdminModal v-model="ruleEditorOpen" kicker="炼丹通用规则" title="编辑炼丹职业通用规则" description="维护炼丹职业的成功率加成和经验收益规则。">
    ...
    <AdminModal v-model="levelEditorOpen" kicker="炼丹等级规则" :title="selectedLevelRule ? `编辑等级 Lv.${selectedLevelRule.level}` : '新建等级规则'" description="维护炼丹师等级经验曲线。">
...
import {
  deleteAdminAlchemyProfessionLevelRule,
  getAdminAlchemyProfessionLevelRule,
  getAdminAlchemyProfessionLevelRules,
  getAdminAlchemyProfessionRule,
  saveAdminAlchemyProfessionLevelRule,
  saveAdminAlchemyProfessionRule
} from '@/services/alchemy-runtime'
import { useAdminPermissions } from '@/composables/useAdminPermissions'
import type { AdminAlchemyProfessionLevelRule, AdminAlchemyProfessionRule } from '@/types/admin'
import toast from '@/utils/toast'

// 页面主状态：
// ruleConfig 是炼丹职业通用规则，levelRules 是等级经验曲线，selectedLevelRule 是当前编辑行。
const { canManageConfigs } = useAdminPermissions()
const levelRules = ref<AdminAlchemyProfessionLevelRule[]>([])
const ruleConfig = ref<AdminAlchemyProfessionRule | null>(null)
const selectedLevelRule = ref<AdminAlchemyProfessionLevelRule | null>(null)
...
async function saveRule() {
  ...
  try {
    ruleConfig.value = await saveAdminAlchemyProfessionRule(ruleConfig.value)
    toast.success('炼丹职业通用规则保存成功。')
    ruleEditorOpen.value = false
    await loadData()
  } catch (error) {
    toast.error(error instanceof Error ? error.message : '炼丹职业通用规则保存失败。')
  }
}

// 打开等级规则编辑弹窗。
async function openEditLevelModal(level: number) {
  selectedLevelRule.value = await getAdminAlchemyProfessionLevelRule(level)
...
async function saveLevelRule() {
  ...
  try {
    selectedLevelRule.value = await saveAdminAlchemyProfessionLevelRule(selectedLevelRule.value)
    toast.success('炼丹职业等级规则保存成功。')
    levelEditorOpen.value = false
    await loadData()
  } catch (error) {
    toast.error(error instanceof Error ? error.message : '炼丹职业等级规则保存失败。')
  }
}
...
async function removeLevelRule() {
  ...
  if (!window.confirm(`确认删除炼丹职业等级 Lv.${selectedLevelRule.value.level} 规则吗？`)) return
  try {
    await deleteAdminAlchemyProfessionLevelRule(selectedLevelRule.value.level)
    toast.success('炼丹职业等级规则删除成功。')
    levelEditorOpen.value = false
    await loadData()
  } catch (error) {
    toast.error(error instanceof Error ? error.message : '炼丹职业等级规则删除失败。')
  }
}
